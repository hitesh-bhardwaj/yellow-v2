...
const CategoryList = ({ categories, activeCategory, setActiveCategory }) => {
  ...
  return (
    <div className='flex gap-[1.5vw] flex-wrap w-full h-fit mb-[5vw]'>
      <Link key={"all"} href="/blog" onClick={() => setActiveCategory('all')} className={`group overflow-hidden relative text-body border hover:text-white transition-all duration-300 ease-out border-body rounded-full px-[1.5vw] text-[1.3vw] py-[0.3vw] leading-[1.2] fadeup ${activeCategory === 'all' ? 'bg-body text-white hover:animate-none' : ''}`}>
