// components/Metadata/RelatedWork.jsx
        ...
        <div className='w-full flex gap-[2vw] mobile:flex-col mobile:gap-[7vw]'>
          {nodes.length > 0 ? (
            nodes.map((node, index) => (
              <div key={node?.slug ?? index} className={`fadeup group w-full h-full ${index === 2 ? "tablet:hidden" : ""}`}>
                <Link href={workPathBySlug(node?.slug ?? '')} className='h-full w-full block relative group-hover:shadow-2xl duration-300 overflow-hidden rounded-xl'>
                  <div className='h-[30vw] w-full relative overflow-hidden mobile:w-full mobile:h-[80vw] tablet:h-[45vw]'>
                    <MediaRender url={node?.workFields?.featuredImagevideo?.node?.mediaItemUrl ?? ''} className='z-[0] group-hover:scale-110 duration-300' />
                  </div>
                  <div className='absolute right-0 left-0 top-0 bottom-0 py-[2vw] px-[1.5vw] flex flex-col justify-between items-start mobile:px-[5vw] mobile:py-[6vw] tablet:p-[3vw]'>
                    <div className="flex gap-[1vw] mobile:gap-[2vw]">
                      {node?.portfolioIndustries?.edges?.slice(0, 2).map((category, i) => (
                    ...
                    <div className='text-white'>
                      <h3 data-title-anim className="text-[2.2vw] drop-shadow-lg font-display mobile:text-[6vw] tablet:text-[3.5vw] leading-[1.2]">
                        {node?.title ?? ''}
                      </h3>
                    </div>
                  </div>
            ...
            <p className="py-10">No related work right now.</p>
          )}
        </div>
      </div>
    </Section>
