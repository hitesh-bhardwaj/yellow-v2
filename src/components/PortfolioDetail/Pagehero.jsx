  ...
  return (
    <>
      <Section className='dark relative w-full h-screen mobile:h-[80vh] tablet:h-[50vh]' id='hero'>
        <span className='absolute top-0 left-0 right-0 bottom-0 bg-black/60' />
        <MediaRender url={src} className='bg-gray-500'/>
        <div className='container absolute top-0 left-0 right-0 bottom-0 z-[1] flex items-center mobile:py-[15%] mobile:flex-col mobile:items-start mobile:justify-end '>
          ...
          <div className="absolute bottom-[7%] right-[7%] flex gap-[1.5vw] text-white mobile:static mobile:gap-[3vw]">
            {tags.slice(0, 2).map((tag) => (
              <p key={tag.slug} className="w-fit border-[1px] capitalize border-white rounded-[60px] backdrop-blur-md px-[1.7vw] py-[0.5vw] text-[1.2vw] fadeup mobile:text-[4vw] mobile:px-[3vw] mobile:py-[1.5vw] bg-white/15 tablet:text-[2vw]">{tag.name}</p>
            ))}
            <p className="w-fit border-[1px] border-white rounded-[60px] backdrop-blur-md px-[2.5vw] py-[0.5vw] text-[1.2vw] fadeup mobile:text-[4vw] mobile:hidden tablet:text-[2vw] bg-white/15">
              {formatDateYear(date)}
            </p>
          </div>
        </div>
