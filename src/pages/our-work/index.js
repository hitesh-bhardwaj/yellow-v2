...
export default function Works({ initialPortfolio, initialPagination, portfolioIndustries }) {
  ...
  useEffect(() => {
    const prefetchNextPage = async () => {
        if (pagination.currentPage < pagination.pagesCount) {
            ...
            const res = await fetch(`/api/getPortfolio?page=${nextPage}`);
  ...
  const loadMorePortfolio = () => {
    ...
    if (nextPageData && !isLoading) {
        setPortfolio((prevPortfolio) => [...prevPortfolio, ...nextPageData.newPortfolio]);
...
export async function getStaticProps({ params }) {
  const { slug } = params || {};
  let { portfolio, pagination } = await getPaginatedPortfolio({
    queryIncludes: 'archive',
  });
  const portfolioIndustries = await getPortfolioIndustries();
  if (slug) {
    const { portfolio: filteredPortfolio, pagination: filteredPagination } = await getPaginatedPortfolio({
      queryIncludes: 'archive',
      categoryId: slug,
    });
