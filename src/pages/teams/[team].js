...
function Teamdetail({ team }) {
  ...
  // Always call hooks (fixes "hooks called conditionally")
  useEffect(() => {
  ...
  // Animations (fine to call every render)
  paraAnim();
  fadeUp();
  lineAnim();

  // If the CMS was unreachable at build-time, team may be null.
  // Render a soft fallback (no 404) so the route stays alive; ISR/fallback will heal it.
  if (!team) {
    return (
      <Layout>
        <Section id="teamdetail" className="w-full h-full bg-black">
          <div className="container py-[8%] bg-white">
            <h1 className="text-[5vw] font-display">This profile is temporarily unavailable</h1>
  ...
  return (
    <>
      <NextSeo
        ...
        languageAlternates={[{ hrefLang: 'x-default', href: `${homepage}/teams/${slug}` }]}
      />
      ...
      <Layout>
        <Section id="teamdetail" className="w-full h-full bg-black">
          <div className="container py-[8%] mobile:pt-[25%] mobile:pb-[15%] tablet:py-[15%] bg-white">
            ...
            {teams?.profilePicture?.node?.sourceUrl && (
...
export async function getStaticProps({ params = {} } = {}) {
  ...
  try {
    ...
    return { props: { team }, revalidate: 500 };
  } catch {
    // Network/CMS error at build time: keep route alive (no 404)
    return { props: { team: null }, revalidate: 60 };
  }
}
