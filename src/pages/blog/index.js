...
export default function Blog({ initialPosts, featuredPost, initialPagination, categories }) {
        ...
        const prefetchNextPage = async () => {
            try {
                if (pagination && pagination.currentPage < pagination.pagesCount) {
                    ...
                    const res = await fetch(`/api/getPosts?page=${nextPage}`, { cache: "no-store" });
                    if (!res.ok) return;
    ...
    const loadMorePosts = () => {
        ...
        setIsLoading(true);
        setPosts((prev) => [...prev, ...nextPageData.newPosts]);
    ...
    const metadata = {
        ...
        description:
            "A blog from Yellow - A Dubai branding and digital agency delivering strategy, identity, advertising and digital solutions to amazing business in the region.",
        img: "home.png",
    ...
    return (
        <>
            ...
            <Layout>
                {featuredPost && <Pagehero post={featuredPost} />}
                <Section id="second-section" className="bg-black">
                    <div className="container py-[5%] bg-white mobile:pb-[15%]">
                        ...
                        <div
                            ref={cardContainer}
                            className="w-full h-full grid grid-cols-3 gap-[2vw] mb-[3vw] mt-[5vw] mobile:grid-cols-1 mobile:gap-[7vw] mobile:mt-[12vw] tablet:grid-cols-2 tablet:mt-[7vw] tablet:gap-x-[3vw] tablet:gap-y-[4vw]"
                        >
                            {posts.map((post, index) => (
                                <div key={post.slug ?? index} className={`${styles.postCard} cardfade`} style={{ animationDelay: `${index * 0.05}s` }}>
                                    <PostCard post={post} />
                                </div>
                            ))}
                            {(!posts || posts.length === 0) && (
                                <p className="text-center col-span-full py-10">No posts available right now.</p>
                            )}
                        </div>
                        ...
                        {pagination && pagination.currentPage < pagination.pagesCount && (
                            <div className="flex w-full justify-center mobile:mt-[7vw]">
                                <button
                                    ...
                                    disabled={isLoading}
                                    className="cursor-pointer flex w-fit relative text-[1.3vw] gap-[0.5vw] items-center font-medium group mobile:text-[6vw] mobile:gap-[2vw] tablet:text-[3vw]"
                                >
...
export async function getStaticProps() {
    // 1) Skip remote fetches entirely on CI (lets build succeed even if WP blocks datacenter IPs)
    if (skipInCI()) {
        return {
            props: {
                initialPosts: [],
            ...
            revalidate: 60, // ISR will repopulate after deploy when hit by users
        };
    }

    // 2) Try to fetch; never throw
    try {
        const { posts: rawPosts = [], pagination = null } = await getPaginatedPosts({ queryIncludes: "archive" });
        const categories = await getCategories().catch(() => []);

        const posts = sortStickyPosts(rawPosts);
        const featuredPost = posts.find((p) => p.isSticky) || null;
        const nonSticky = posts.filter((p) => !p.isSticky);

        return {
            props: {
                initialPosts: nonSticky,
                featuredPost,
                categories,
                initialPagination: pagination
                    ? { ...pagination, basePath: "/blog" }
                    : null,
            },
            // Use ISR so the page can recover even if WP blips after deploy
            revalidate: 300,
        };
    } catch {
        return {
